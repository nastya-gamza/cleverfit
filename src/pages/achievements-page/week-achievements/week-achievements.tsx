...
import {trainingInfoCards} from '@constants/training-info-cards.ts';
...
import {generateExerciseDataByDay} from '@utils/achievements/generate-exercise-data-by-day.ts';
...
export const WeekAchievements = () => {
    ...
    const averageLoadByDayOfWeek = calculateAverageLoadByDay(sortedDataByDayOfWeek, true);
    ...
    const sortedMostFrequentExercises = sortByDayOfWeek(mostFrequentExercisesCount);

    const exercisesPercentage = calculateExercisePercentages(mostFrequentExercisesCount);

    const mostFrequentExercisesByDayOfWeek = generateExerciseDataByDay(sortedMostFrequentExercises);
        ...
        <React.Fragment>
            ...
            <div className={styles.trainingLoad}>
                ...
                <div className={styles.infoByDays}>
                    ...
                    <List
                        dataSource={mostFrequentExercisesByDayOfWeek}
                        renderItem={(el, i) => (
                            <List.Item>
                                <Badge
                                    count={i + 1}
                                    text={<Text type='secondary'>{el.day}</Text>}
                                />
                                <Text strong={true}>{el.value && `${el.value}`}</Text>
                            </List.Item>
                        )}
                        split={false}
                        className={styles.list}
